...
const STORAGE_KEY = "iphone_chat_saved_scenes_v2";
...
function buildSceneFromUI(){
  return { meta: getMetaFromInputs(), messages: extractMessagesFromBuilder() };
}
...
function populateMetaInputs(meta){
  ...
  els.videoFps.value = String(meta.videoFps ?? 12);
  els.videoScale.value = String(meta.videoScale ?? 1.5);
...
function createBuilderRow(data = {}){
  ...
  row.querySelector(".move-up-btn").addEventListener("click", ()=>{
    ...
    if(prev){ els.messageBuilder.insertBefore(row, prev); syncJsonFromUI(); }
  });
  row.querySelector(".move-down-btn").addEventListener("click", ()=>{
    const next = row.nextElementSibling;
    if(next){ els.messageBuilder.insertBefore(next, row); syncJsonFromUI(); }
  });
...
function sleep(ms){
  return new Promise((resolve)=>{
    const tick = ()=>{
      if(isPaused){ pendingResolve = ()=>tick(); return; }
...
function waitWhilePaused(){
  return new Promise((resolve)=>{
    const check = ()=>{ if(!isPaused) return resolve(); activeTimeout = setTimeout(check, 100); };
    check();
  });
}
...
function scrollMessagesToBottom(instant = false){
  els.messagesScroll.scrollTo({ top: els.messagesScroll.scrollHeight + 9999, behavior: instant ? "instant" : "smooth" });
}
...
function pressKeyVisual(char){
  const target = [...els.keyboard.querySelectorAll("button")].find((btn)=>btn.dataset.key === char);
  if(!target) return;
...
function textToKeys(text){
  return [...text].map((ch)=> ch === " " ? "space" : ch === "\n" ? "return" : ch);
}

function ensureAudio(){
  if(!soundEnabled || isRenderingVideo) return null;
...
function beep({frequency = 660, duration = 0.04, type = "sine", gain = 0.03} = {}){
  ...
  osc.type = type; osc.frequency.value = frequency; g.gain.value = gain;
...
async function runScene(scene){
  ...
  if(runDelay > 0){
    ...
    els.countdown.textContent = isRenderingVideo ? "REC" : "RUN";
    await sleep(600);
  ...
  for(const msg of scene.messages.map(sanitizeMessage)){
    ...
    if(msg.showTyping){
      if(isSelf){
        ...
        if(msg.sound !== "none" && !isRenderingVideo){
  ...
  hideTyping();
  clearComposer();
  setStatus(isRenderingVideo ? "ვიდეო მზადდება..." : "Done — შეგიძლია ისევ გაუშვა");
}

function applyJsonToBuilderAndPreview(){ loadScene(parseSceneJson()); }
...
function downloadSceneJson(){
  ...
  a.download = `${(els.sceneName.value || "scene").trim()}.json`;
  a.click();
...
function copySceneJson(){
  ...
  navigator.clipboard.writeText(els.sceneJson.value)
    .then(()=>setStatus("JSON დაკოპირდა"))
    .catch(()=>setStatus("ვერ დაკოპირდა"));
}

function toggleRecordMode(){
  document.body.classList.toggle("record-mode");
  setStatus(document.body.classList.contains("record-mode") ? "Record Mode ON" : "Record Mode OFF");
}
...
function bindMetaInputEvents(){
  [
    ...
    els.videoFps, els.videoScale
  ].forEach((input)=>{
    input.addEventListener("input", ()=>{ applyMetaToPreview(getMetaFromInputs()); syncJsonFromUI(); });
    input.addEventListener("change", ()=>{ applyMetaToPreview(getMetaFromInputs()); syncJsonFromUI(); });
  });
}
...
function refreshSavedScenesSelect(){
  const scenes = getSavedScenes();
  const keys = Object.keys(scenes);
  els.savedScenes.innerHTML = "";
  if(!keys.length){
    const opt = document.createElement("option");
    opt.value = "";
    opt.textContent = "ჯერ არაფერია შენახული";
    els.savedScenes.appendChild(opt);
    return;
  }
  keys.forEach((key)=>{
    const opt = document.createElement("option");
...
function saveCurrentScene(){
  ...
  if(!name){ alert("სცენის სახელი ჩაწერე"); return; }
  const scenes = getSavedScenes();
  scenes[name] = buildSceneFromUI();
  setSavedScenes(scenes);
  refreshSavedScenesSelect();
  els.savedScenes.value = name;
  setStatus(`სცენა შენახულია: ${name}`);
}

function loadSelectedSavedScene(){
  const name = els.savedScenes.value;
  const scenes = getSavedScenes();
  if(!name || !scenes[name]) return;
  els.sceneName.value = name;
  loadScene(scenes[name]);
  setStatus(`სცენა ჩაიტვირთა: ${name}`);
}

function deleteSelectedSavedScene(){
  const name = els.savedScenes.value;
  if(!name) return;
  const scenes = getSavedScenes();
  ...
  setSavedScenes(scenes);
  refreshSavedScenesSelect();
  setStatus(`სცენა წაიშალა: ${name}`);
}

async function renderStageToCanvas(canvas, scale = 1.5){
  const snap = await html2canvas(els.tiktokStage, {
  ...
  const srcRatio = snap.width / snap.height;
...
async function renderVideoAndDownload(){
  ...
  if(typeof html2canvas === "undefined"){
    alert("ვიდეო რენდერის ბიბლიოთეკა ვერ ჩაიტვირთა.");
    return;
  }
  ...
  setStatus("ვიდეოს რენდერი დაიწყო...");
  ...
  const captureLoop = async ()=>{
    while(isRenderingVideo && isRunning){
      ...
      await new Promise((r)=>setTimeout(r, Math.max(30, 1000 / fps)));
    }
  };
  ...
  try{
    ...
    console.error(error);
    alert("ვიდეოს რენდერისას შეცდომა მოხდა.");
    setStatus("ვიდეოს რენდერი ვერ დასრულდა");
  }finally{
...
els.sceneFileInput.addEventListener("change", async (event)=>{
  ...
  }catch{ alert("ფაილი ვერ წავიკითხე."); }
...
document.addEventListener("keydown", (event)=>{
  if(event.code === "Space" && event.target.tagName !== "TEXTAREA" && event.target.tagName !== "INPUT"){
    ...
    if(isRunning){
      ...
      if(!isPaused && pendingResolve){ const resolver = pendingResolve; pendingResolve = null; resolver(); }
    }
  }
});
